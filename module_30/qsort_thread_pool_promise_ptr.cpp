...
void quicksort(int* array, long left, long right, std::shared_ptr<std::promise<void>> counter = nullptr)
{
	...
	if (make_thread_pool && (right_bound - left > 100'000))
	{
		if (counter)
		{
			tpool.push_request(quicksort, array, left, right_bound, counter);
			quicksort(array, left_bound, right, counter);
		}
		else
		{
			auto p_ptr = std::make_shared<std::promise<void>>(std::promise<void>());
			tpool.push_request(quicksort, array, left, right, p_ptr);
			quicksort(array, left_bound, right, p_ptr);
			p_ptr->get_future().wait();
		}
	}
	else if (make_thread && (right_bound - left > 100'000))
	{
	...
	{
		quicksort(array, left, right_bound, counter);
		quicksort(array, left_bound, right, counter);
	}

	if (counter && (right - left > 100'000) && (counter.use_count() == 3)) counter->set_value();
}
