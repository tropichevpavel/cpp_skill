...
void first_thread(std::list<Part::PartPtr>& parts, std::promise<void>& p)
{
...
void second_thread(const std::future<bool>& first_thread_future)
{
...
int main()
{
	...
	std::thread t1(first_thread, std::ref(spare_parts), std::ref(p1));
	std::thread t2(second_thread, p1.get_future());
